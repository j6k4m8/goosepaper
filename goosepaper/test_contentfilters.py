...
def test_css_skip_filter_removes_matching_elements():
    ...
    result = apply_content_skip_filters(html, [{"type": "css", "selector": "div.ad"}])
...
def test_regex_skip_filter_strips_matching_text():
    ...
    result = apply_content_skip_filters(html, [{"type": "regex", "pattern": "Mehr anzeigen"}])
...
def test_regex_skip_filter_respects_flags():
    ...
    result = apply_content_skip_filters(
...
def test_apply_content_skip_filters_with_no_filters_returns_html_unchanged():
    html = "<p>Untouched</p>"
    assert apply_content_skip_filters(html, []) == html
    assert apply_content_skip_filters(html, None) == html
